void autos() {
  switch (state) {
    ...
    case SETTINGS3:
      //enc1.tick();
      if (enc1.isRight()) settings.turnover_liter = 4;
      if (enc1.isLeft()) settings.turnover_liter = 2;
    ...
    case START:
      if (settings.total_rev)
      {
      if ((counter / settings.turnover_liter) >= settings.total_rev) {
        state = STOP;
      }
      }
      if (enc1.isRight()) settings.speed += 1;
